...
//! Uses: `crate::core::filter::EntryFilter` (path selection), `crate::core::models::Entry`,
//! `crate::core::source::Source`, and the [`compare`] submodule.
//!
...
use crate::core::models::{ContentDiff, Entry};
use crate::core::source::Source;
use crate::engine::is_selected;
...
/// sides to report what changed *inside* it (see [`crate::formats::inspect::diff`]).
pub fn diff_sources(
    a: &dyn Source,
    b: &dyn Source,
    mode: CompareMode,
...
/// (the shared [`is_selected`] rule), keyed by internal path for pairing.
fn comparable<'s>(src: &'s dyn Source, filter: &EntryFilter) -> BTreeMap<&'s str, &'s Entry> {
    src.entries()
        .iter()
        .filter(|e| is_selected(e, filter))
