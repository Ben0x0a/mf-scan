...
//! nested `.zip` (read into an in-memory arena; see `crate::core::source::nested`).
...
//! Uses: `crate::core::models::{Entry, Location}`, `crate::core::source::{nested, zip}`, `std::fs`.
//!
...
use crate::core::source::{Content, Source};
use crate::core::source::{nested, zip};
...
impl Source for FolderSource {
    ...
    fn content(&self, entry: &Entry) -> Result<Content<'_>> {
        match &entry.location {
            ...
            Location::Zip { .. } | Location::RangedZip { .. } | Location::Tar { .. } => bail!(
                "FolderSource::content called on a top-level archive entry: {}",
                entry.name
            ),
        }
    }
