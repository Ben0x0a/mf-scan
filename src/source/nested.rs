...
mod tests {
    ...
    /// Build a minimal STORED-only ZIP archive in memory from `(name, data)` pairs.
    ///
    /// WHY a builder lives here: unit tests cannot see the `tests/common` helper
    /// (it is compiled into the integration-test binaries, not the library), and
    /// these tests need to fabricate exact nested layouts. STORED-only keeps it
    /// tiny — the parser reads the local header, central directory and EOCD the
    /// same way regardless of method, so STORED exercises the path we care about.
    fn build_stored_zip(files: &[(&str, &[u8])]) -> Vec<u8> {
        fn u16(buf: &mut Vec<u8>, v: u16) {
        ...
        for (name, data) in files {
            ...
            u32(&mut buf, 0x0403_4b50); // local file header signature
            u16(&mut buf, 20); // version needed
            u16(&mut buf, 0); // flags
            u16(&mut buf, 0); // method 0 = STORED
            u16(&mut buf, 0); // mod time
            u16(&mut buf, 0); // mod date
            u32(&mut buf, 0); // crc32 (parser ignores it)
            u32(&mut buf, data.len() as u32); // compressed size
            u32(&mut buf, data.len() as u32); // uncompressed size
            u16(&mut buf, name.len() as u16);
            u16(&mut buf, 0); // extra length
        ...
        for (i, (name, data)) in files.iter().enumerate() {
            u32(&mut buf, 0x0201_4b50); // central directory header signature
            u16(&mut buf, 20); // version made by
            u16(&mut buf, 20); // version needed
            u16(&mut buf, 0); // flags
            u16(&mut buf, 0); // method 0 = STORED
            u16(&mut buf, 0); // mod time
            ...
            u16(&mut buf, name.len() as u16);
            u16(&mut buf, 0); // extra length
            u16(&mut buf, 0); // comment length
            u16(&mut buf, 0); // disk number start
            u16(&mut buf, 0); // internal attributes
            u32(&mut buf, 0); // external attributes
            u32(&mut buf, local_offsets[i]); // local header offset
            buf.extend_from_slice(name.as_bytes());
        }
        let cd_size = buf.len() as u32 - cd_start;
        // End of central directory.
        u32(&mut buf, 0x0605_4b50); // EOCD signature
        u16(&mut buf, 0); // disk number
        u16(&mut buf, 0); // disk with central directory
        u16(&mut buf, files.len() as u16); // entries on this disk
        u16(&mut buf, files.len() as u16); // total entries
        u32(&mut buf, cd_size);
        u32(&mut buf, cd_start);
        u16(&mut buf, 0); // comment length
        buf
    }
