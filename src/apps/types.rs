...
//! Used by: [`crate::apps::catalog`] (builds and queries these) and the per-platform
//! resolvers ([`crate::apps::ios_ffs`], [`crate::apps::ios_backup`],
//! [`crate::apps::android`], which construct [`AppContainer`]s).
//! Uses: nothing beyond `std` — these are plain data types, kept in their own
//! module so the resolvers and the catalogue depend on the data shape, not on each
//! other.
